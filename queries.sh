...
echo "$($PSQL "SELECT MAX(winner_goals) FROM games LIMIT 1")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT count(winner_id) FROM games where winner_goals > 2")"

echo -e "\nWinner of the 2018 tournament team name:"
echo "$($PSQL "select teams.name from teams full join games on teams.team_id = games.winner_id where round like 'Final' and year = 2018")"
